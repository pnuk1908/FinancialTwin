...
# Path to the Excel file and shapefile data
...
# Call get_financial_twin for an initial dataset
...
def index():
    try:
        ...
        table_html = merged_df.to_html(classes='table table-striped', index=False, escape=False, formatters={
...
def export_maps_data():
    try:
        # Assuming the maps are stored in the 'static' folder
...
def find_closest_and_farthest_factors(municipality, financial_twin, financial_df, type, num_factors=5):
    try:
        ...
        # Extract the data for the selected municipality and its financial twin
        ...
        # Generate list of column names starting from the 8th column (index 7)
        ...
        # Extract numeric data for the selected municipality and twin
        ...
        # Calculate the absolute differences for each financial parameter
        differences = np.abs(municipality_data.values - twin_data.values).flatten()
        
        # Create a DataFrame to hold the differences
        #differences_df = pd.DataFrame(data=differences, index=columns_to_compare, columns=['Difference'])

        # Create a DataFrame to hold the differences
        differences_df = pd.DataFrame(data={
            'Difference': differences,
            'Value_Municipality': municipality_data.values.flatten(),
            'Value_Twin': twin_data.values.flatten(),
        }, index=columns_to_compare)
        
        # Identify the closest and farthest factors based on the provided num_factors
...
def download_excel():
    try:
        ...
        return send_file(output, as_attachment=True, download_name='analysis_factors.xlsx', mimetype='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
...
def analysis(twin, type, year, per_capita):
    ...
    # Store the results in the session for reuse
    session['closest_factors'] = closest_factors.to_dict()
    session['farthest_factors'] = farthest_factors.to_dict()

    return render_template('analysis.html', municipality=municipality, twin=twin, closest_factors=closest_factors, farthest_factors=farthest_factors, num_factors=num_factors, year=year,include_per_capita=per_capita)
...
def generate_maps(financial_data_type, num_clusters, include_population, selected_fields):
    ...
    try:
        ...
        # Select the appropriate DataFrame based on the financial data type and population inclusion
        ...
        if include_population:
            # Modify selected_fields to include the '_relative' suffix
            selected_columns = [field + '_relative' for field in selected_fields]

        else:
            selected_columns = selected_fields
            
        # Combine the lists, ensuring required_columns appear first and avoid duplicates
        ...
        # Load the shapefile data for the map
        #shapefile_path = os.path.join(shapefile_folder, 'dvg2gem_nw.shp')
        ...
        for year in years:
            ...
            for x, y, label in zip(gdf_yearly.geometry.centroid.x, gdf_yearly.geometry.centroid.y, gdf_yearly['Municipality']):
            ...
            #plot_path = os.path.join('static', f'map_{year}.png')
            plt.savefig(plot_path)
            plt.close()
            maps.append(filename)

            # Save the GeoDataFrame to a CSV file for each year
        ...
        print(err)              # console
        return None, err        # return error to caller
...
def normalize_data(df):
    ...
    columns_to_compare = all_columns[7:-1]  # Modify based on your specific dataset
    normalized_data = scaler.fit_transform(df[columns_to_compare])
    return normalized_data


# Function to calculate ideal number of clusters
def calculate_ideal_clusters(final_df, year_ideal):
    try:
        ...
        for year in years:
            ...
            #ideal_num_clusters = max(silhouette_scores, key=silhouette_scores.get)
            # Identify the key with the value closest to 1
    ...
    if request.method == 'POST':
        try:
            ...
            if action == 'calculate_clusters1' or action == 'calculate_clusters2':
                ...
                # Modify selected fields if per capita is included
                if include_population:
                ...
                if action == 'calculate_clusters2':
                    ...
                    ideal_num_clusters, ideal_num_clusters_year = calculate_ideal_clusters(final_df, None)

                # Re-render the form with the calculated ideal number of clusters
                ...
                #maps, csv_files, years = generate_maps(financial_data_type, num_clusters, include_population, selected_fields)
                ...
                if result is None or result[0] is None:
                    ...
                    flash("Map generation failed: {err}", "error")
                    return render_template("error.html"), 500
...
def view_maps():
    # Retrieve maps and years from query string
